...
In this report, we study the effects of government polices on COVID-19 pandemic control. We use everyday new COVID-19 cases in year 2021 as response variable, key explanatory variables are one country indicator variable, and four policy variables (mask policy, stay-at-home policy, vaccine policy, international travel policy). We build factorial ANOVA model based on normality assumption, and conduct hypothesis tests, as well as construct confidence intervals. Conclusions of our analysis can serve as a reference for the general public.
...
*inter_travel* ($\zeta$): Record restrictions on international travel (for foreign travelers, not citizens). Five levels: 0 - no restrictions, 1 - screening arrivals, 2 - quarantine arrivals from some or all regions, 3 - ban arrivals from some regions, 4 - ban on all regions or total border closure.
...
Some important statistics for key variables we are interested are:
...
After an worldwide overview, our analysis after will be limited in five major European countries: Germany, United Kingdom, France, Italy and Spain. This decision is based on following reasons: 
...
* 2. The five countries are all developed countries and have similar culture background, and this can reduce the effect brought by countries, which let us focus on the effects brought policies. 

* 3. Most people are able to travel freely (no need of a visa) from one country to another, which makes the effect of `internationaltravel` easier to observe. 
...
Before building an ANOVA model, we first make main effect plots for four policy variables (variable country is not our focus here). From the graph, it seems that in each policy variable, there are some groups having significant difference in `log(new_casesL20`. 
...
Here we do not consider the interaction term between `stay_home` and `vac_policy`, as this one has no useful meaning. The ANOVA table below shows the hypothesis test result for each interaction term. Take `facial_policy*stay_home` as an example, we have $(\beta\delta)_{jl}=0$ v.s.
$H_1: {\rm not \ all\ } (\beta\delta)_{jl}\ {\rm are\ zero}$. Again, with full model and reduced model, we can construct F-statistics. Since $p=0.175$, we can not reject null hypothesis under significant level $\alpha=0.05$. For the rest interaction terms, we all reject the null hypothesis, and consider them significant.
...
We construct 95% confidence interval using Tukey-Kramer method. Here, we only display that for `facial_policy` and `stay_home`. As others have too many levels, making the plots meaningless. 
...
We also need to conduct model diagnostics to check model assumption. The major assumption we make is that $\epsilon_{ijklmn}$ are i.i.d $N(0,\sigma^2)$. This contains two sub assumptions: 1. error term has equal variance; 2. the error term follows normal distribution. 
...
To inspect normality assumption, we inspect out Normal Q-Q plot. We can see the standardized residuals fit with theoretical quantiles pretty well. There is no violation of normality assumption. This is mainly because we first conduct log transformation on response variable `New_cases`, which greatly improve the situation of right-skewed distribution. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
plot(anova.fit, which = 2)
```

As the model assumptions are not violated, our procedures of conducting hypothesis tests and building confidence intervals seem to be valid here. We can come up with some helpful conclusions.
...
* In particular, facial policy, vaccine policy, and stay-at-home policy all help reduce COVID new cases. However, the effects are different with regard to the countries.
...
There are still drawbacks in our analysis. First is that although by introducing interaction terms, we are able to explain more sample variances, there are still large parts that are not explained. As COVID new cases can actually be affected by many factors, like social economic factors, environmental factors, nature of virus. It is impossible for us to exhaust all of them in this analysis. Also, though we are able to draw solid conclusions based confidence intervals for facial policy and stay-at-home policy, we can not reach a similar one for vaccine policy. We only know that it does have effect on COVID new cases. Moreover, it is always debatable on how policies and new cases interact with each other. Is it because pandemic becomes severe that government publishes stricter policies, or stricter policies help control the pandemic? In our analysis, we tackle this problem by taking lag on response variable, but some counterintuitive facts still show up, and need further study.

# Acknowledge

We want to thank Dr. Chen for introducing ANOVA model to us, as well as providing the WHO data set. We also want to thank our project adviser Zitong for providing helpful advice. Besides, I want to thank my teammates Xiaoyi, Tianyi and Kay for discussing projects together.
